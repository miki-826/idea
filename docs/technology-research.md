...
- Date: 2026-06-23 JST
- Installed versions: Next.js 16.2.9, React 19.2.4, TypeScript 5, `openai`, `@google/genai`, `@supabase/supabase-js` 2.108.2
...
- OpenAI API docs, Responses API and Structured Outputs: https://developers.openai.com/api/docs/guides/structured-outputs
...
| OpenAI API via `openai` | ADOPT | User requested ChatGPT/OpenAI API key support; Structured Outputs fit IdeaSpec. | `buildMockSpec` on no key, timeout, invalid JSON, or API error. | POST `/api/ai/analyze` with and without key |
| Gemini API via `@google/genai` | KEEP optional | Previously planned provider; structured JSON fits IdeaSpec. | `buildMockSpec` on no key, timeout, invalid JSON, or API error. | POST `/api/ai/analyze` with and without key |
...
OpenAI external calls are server-only and gated by `OPENAI_API_KEY`. The UI never receives the key. The route uses Responses API Structured Outputs with a JSON schema and falls back after 9 seconds. Provider selection uses `AI_PROVIDER=openai`; if no provider is set, OpenAI is preferred when `OPENAI_API_KEY` exists.
